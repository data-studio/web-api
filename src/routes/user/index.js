...
module.exports = function (dataStudio) {
    ...
    verifyAuthN(req.body.Login, req.body.Password)
      .then(function (user) {
        ...
        createToken(user, 86400)
          .then(function (token) {
            returnAuthAttempt(res, attemptId);
          })
          .catch(function (err) {
            console.log(err);
  ...
  function returnAuthAttempt (res, attemptId) {
    db.fetchAuthAttemptById(attemptId)
      .then(function (authAttempt) {
        res.returnNewObject(authAttempt);
      })
      .catch(function (err) {
        console.log(err);
        reject(new Error("ERR_LOGIN_FAILED: Failed to retrieve auth attempt"));
      });
  }
